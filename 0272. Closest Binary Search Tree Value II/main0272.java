...
/*
1. maxheap abs(val - target), k
size < k, add root.val
     >= k,  peek() > val, replace

O(N*logK)

2. quickSelect  o(N)  -> traversal -> list

a. partition, random select,   smaller    pivot
   [0, pivot_idx]  pivot_idx == k
   
           piv_idx                  |   
   3(0.7), 4(0.3), 2(1.7), 1(2.7), 5(1.3)
    ^                  |
...
3. search closet in bst: log(n) + k
   node
   iterator^() tc:o(1)  stack sc:o(n)
   iteratorv() tc:o(1)        sc:o(n)
